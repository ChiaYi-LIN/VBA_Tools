...
Set countryOne = Application.InputBox("Select the Header Cell (If the location level is city or state, then choose the corresponding header cell)", "First Country Range Select", Type:=8)
...
Set countryTwo = Application.InputBox("Select the Header Cell (If the location level is city or state, then choose the corresponding header cell)", "Second Country Range Select", Type:=8)
...
Sub IndicateMasterAndAliased(ByVal companyName As Range, ByVal OIDName As Range)
...
For j = startRow To endRow
    ...
    If Cells(j, 4).Value = "Master" Then
        ...
        Cells(j, 6).Value = "Not " & LocationLevel Then
            ...
            LookIn:=xlValues, lookat:=xlWhole, after:=Cells(startRow, 9))
...
For j = startRow To endRow
    ...
    For k = startRow To endRow
        If Cells(k, 4).Value = "Aliased" And Cells(k, 7).Value = "Has GID" Then
            Cells(k, 2).Value = "Check Again (Aliased Has GID)"
        End If
    Next k
    End If
    
    If Cells(j, 8).Value = "At Least One Difference" Then
        If Cells(j, 2).Value <> "" Then
            Cells(j, 2).Value = Cells(j, 2).Value & "(Information Difference)"
        Else
            Cells(j, 2).Value = "Check Again (Information Difference)"
        End If
    End If
Next j
...
reCACC, reStat, reIfDup, reSim As Long
reComment = 2
reMOID = 3
reMName = 4
reMLoc = 5
...
Worksheets("Results").Cells(1, rePar).Value = "Parent Customer Name"
Worksheets("Results").Cells(1, reGID).Value = "GID"
...
k = reType
For Each col In theInformation.Columns
    Worksheets("Results").Cells(1, k).Value = Worksheets("Master & Aliased").Cells(1, col.Column).Value
    k = k + 1
Next col
reSim = k
Worksheets("Results").Cells(1, reSim).Value = "Similarity"
reIfDup = k + 1
Worksheets("Results").Cells(1, reIfDup).Value = "Duplicated company name and location check"
...
Sub CreditCard(ByVal companyName As Range, ByVal OIDName As Range)

Dim currentRow, countRowNum, countColNum, i, j, mcompanyNameCol, startRow, endRow, minOID, OIDNameCol As Long
Dim currentCompanyName As String
    
Worksheets("Master & Aliased").Activate
countRowNum = Cells(1, 2).CurrentRegion.Rows.Count
...
Cells(1, 3).Value = "OID to be changed to"
Cells(1, 4).Value = "Master or Aliased"
Cells(1, 5).Value = "Real Display Name"
...
For i = 2 To countRowNum
    Cells(i, 5).Value = Worksheets("Source").Cells(Worksheets("Source").Columns(6).Find(Cells(i, OIDNameCol).Value, _
    LookIn:=xlValues, lookat:=xlWhole).Row, 2).Value
...
countColNum = Cells(1, 2).CurrentRegion.Columns.Count
For j = 2 To countColNum
    Worksheets("Results").Cells(1, j).Value = Cells(1, j).Value
Next j
...
For j = 2 To countColNum
    Worksheets("Results").Cells(currentRow, j).Value = Cells(i, j).Value
Next j
currentRow = currentRow + 1
End If
Next i
...
Worksheets("Results").Columns("D:E").Insert Shift:=xlToRight
Worksheets("Results").Columns("G").Insert Shift:=xlToRight

Cells(1, 3).Value = "Master OID"
Cells(1, 4).Value = "Master Company Name"
Cells(1, 5).Value = "Master Location"
Cells(1, 7).Value = "Aliased OID"
Cells(1, 8).Value = "Aliased Company Name"

For i = 2 To countRowNum
currentRow = Worksheets("Source").Columns(6).Find(Cells(i, 13).Value, _
LookIn:=xlValues, lookat:=xlWhole).Row
Cells(i, 4).Value = Worksheets("Source").Cells(currentRow, 2).Value
Cells(i, 5).Value = Worksheets("Source").Cells(currentRow, 3).Value
Cells(i, 7).Value = Cells(i, 13).Value
Next i

Columns(9).EntireColumn.Delete
Cells(1, 9).Value = "Aliased Location"
